...
- Convite: RPC `create_workspace_invite(workspace_id, email, role)` gera token e prazo; a Edge `accept-invite` valida Bearer no handler, confere token, expiração, igualdade do e-mail do convite com o usuário autenticado e insere o membership com `service_role` (gateway com `verify_jwt = false` para o preflight CORS não falhar no navegador).
...
- **CORS no `accept-invite` chamado do browser (Vercel):** com **`verify_jwt = true`** no gateway, o preflight **OPTIONS** não carrega `Authorization` e a resposta da borda não era HTTP OK, bloqueando `fetch` de origem `https://polaris-crm.vercel.app`. **Solução:** `verify_jwt = false` para `accept-invite` em `supabase/config.toml`, validação de sessão apenas no handler (Bearer + `getUser`), cabeçalhos CORS com `Access-Control-Allow-Methods`; redeploy da função com `supabase functions deploy accept-invite`.
- **Onboarding mostrando “Perfil: owner” para convidado `member`:** a política de `select` em `workspace_members` permite ler todas as linhas do workspace; a listagem em `/onboarding/workspace` não filtrava `user_id`, então a primeira linha por `created_at` costumava ser a do dono. **Solução:** `select` com `.eq("user_id", session.user.id)` para listar apenas as memberships do usuário logado.
...
- **Gatilho de etapa no local sem sugestões automáticas:** o trigger `pg_net` existe, mas **`lead_stage_webhook_secret`** em `app_runtime_config` começa vazio — sem `UPDATE` alinhado ao `LEAD_STAGE_WEBHOOK_SECRET`, não há `POST` na Edge. **Solução:** SQL no Studio local conforme `supabase/README.md` (seção testes locais) e `roadmap/08-gatilho/61-supabase-local-edges-env-webhook-pgnet.md`. Em paralelo, o dashboard chama **`notifyLeadStageAutoGeneration`** após **`transition_lead_stage_atomic`** com JWT na mesma Edge, para gerar sugestões mesmo sem webhook ou secret preenchidos; o dedupe evita duplicar quando o banco também dispara.
...
- **Logout pelo menu não levava a `/login` de forma confiável:** após `signOut`, `router.push`/`refresh` no App Router às vezes não atualizava a navegação como o usuário esperava. **Solução:** após `signOut`, usar `window.location.replace("/login")` para forçar recarga completa e estado de cliente limpo.
...
- **E-mail de confirmação de cadastro genérico:** o template padrão não seguia a identidade visual do app. **Solução:** `supabase/templates/confirmation.html` com o mesmo layout do recovery (logo, cartão, cores), botão e URL em texto para `{{ .ConfirmationURL }}` e bloco de `{{ .Token }}`, registrado em `[auth.email.template.confirmation]` no `config.toml`; na nuvem, copiar HTML e assunto para **Confirm signup** no painel.
...
- **Reordenação de etapas e unicidade de `position`/`slug`:** um único `UPDATE` de posições podia violar `unique (workspace_id, position)` durante o commit; nomes distintos podiam colidir no `slug`. **Solução:** RPC `reorder_funnel_stages` em duas fases com posições temporárias fora do intervalo atual (`max(position) + 100000 + ordem`), `FOR UPDATE` no workspace para concorrência, depois posições finais; sufixo automático no `slug` em criação/renomeação; UI sem remover etapas `is_system`.
- **Workspace salvo no browser após reset do banco:** `localStorage` mantinha `workspace_id` antigo sem membership, e o Kanban listava zero etapas. **Solução:** hook `useResolvedWorkspaceId` valida o id contra `workspace_members` após a sessão e limpa a chave se for inválido; onboarding e accept-invite usam a mesma constante de chave.
...
- [x] Template de e-mail de confirmação de cadastro alinhado ao recovery (logo, cartão, link + código OTP, PT-BR)
...
- [x] Rótulos em português para papéis de workspace na UI (`owner` e `admin` como **Administrador**, `member` como **Membro**), com helper reutilizável e testes unitários
- [x] Lista de membros para filtro de responsável e campo **Responsável** no lead via RPC `list_workspace_members_for_assignee` (e-mail + papel), rótulo `local-do-email(papel)` em minúsculas
- [x] Remoção de workspace por owner via RPC segura (`delete_workspace_as_owner`) com ação na tela de onboarding
- [x] Base de testes com Jest (unitário) e Cypress (integração), com gate de cobertura mínima de 91% para unitários
